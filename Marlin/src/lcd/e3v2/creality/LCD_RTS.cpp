...
void RTSSHOW::RTS_Init()
{
  ...
  AxisUnitMode = 1;
    ...
    for (int y = 0; y < GRID_MAX_POINTS_Y; y++)
    {
      ...
      {
        RTS_SndData(z_values[x][y] * 100, AUTO_BED_LEVEL_1POINT_VP + showcount * 2);
        showcount++;
      }
    }
...
void RTSSHOW::RTS_HandleData()
{
  ...
  switch(Checkkey)
  {
    ...
    case AdjustmentKey:  //调整界面
      ...
      // else if(recdat.data[0] == 3)//控制风扇开启
      // {
      ...
      //     queue.enqueue_now_P(PSTR("M106 S255"));
    ...
    case SettingScreenKey://设置界面
      ...
      {
        char sizebuf[20] = {0};
        sprintf(sizebuf, "%d X %d X %d", X_MAX_POS-10 , Y_MAX_POS , Z_MAX_POS);
    ...
    case BedLevelFunKey://调平界面
      ...
      if (recdat.data[0] == 6)
      {
        ...
        if(!planner.has_blocks_queued())
        {
          ...
          queue.enqueue_now_P(PSTR("G1 X150 Y127.5 F3000"));
      ...
      else if (recdat.data[0] == 7)
      {
        ...
        if(!planner.has_blocks_queued())
        {
          waitway = 4;
          queue.enqueue_now_P(PSTR("G1 F600 Z3"));
          queue.enqueue_now_P(PSTR("G1 X50 Y45 F3000"));
          queue.enqueue_now_P(PSTR("G1 F600 Z0"));
          waitway = 0;
        }
      }
      else if (recdat.data[0] == 8)
      {
        ...
        if(!planner.has_blocks_queued())
        {
          waitway = 4;
          queue.enqueue_now_P(PSTR("G1 F600 Z3"));
          queue.enqueue_now_P(PSTR("G1 X250 Y45 F3000"));
          queue.enqueue_now_P(PSTR("G1 F600 Z0"));
          waitway = 0;
        }
      }
      else if (recdat.data[0] == 9)
      {
        ...
        if(!planner.has_blocks_queued())
        {
          waitway = 4;
          queue.enqueue_now_P(PSTR("G1 F600 Z3"));
          queue.enqueue_now_P(PSTR("G1 X250 Y210 F3000"));
          queue.enqueue_now_P(PSTR("G1 F600 Z0"));
          waitway = 0;
        }
      }
      else if (recdat.data[0] == 10)
      {
        ...
        if(!planner.has_blocks_queued())
        {
          waitway = 4;
          queue.enqueue_now_P(PSTR("G1 F600 Z3"));
          queue.enqueue_now_P(PSTR("G1 X50 Y210 F3000"));
          queue.enqueue_now_P(PSTR("G1 F600 Z0"));
          waitway = 0;
        }
      }
      ...
      if (recdat.data[0] == 1)//PID 
        {
          ...
          hot_i = (PID_PARAM(Ki, 0)/16*10000)+0.00001;
          hot_d = PID_PARAM(Kd, 0)*16;
          
          bed_p = thermalManager.temp_bed.pid.Kp*100;
          bed_i = (thermalManager.temp_bed.pid.Ki/16*10000)+0.0001;
          bed_d = thermalManager.temp_bed.pid.Kd*1.6;
    ...
    break;                      
    case Nozzle_I: 
      PID_PARAM(Ki, 0) = (float)recdat.data[0]*16/10000;
      thermalManager.updatePID();
    break;                      
    case Nozzle_D:
      PID_PARAM(Kd, 0) = (float)recdat.data[0]/16;
      thermalManager.updatePID();
    break;                      
    case Hot_Bed_P:
    ...
    break;                      
    case Hot_Bed_I:
      thermalManager.temp_bed.pid.Ki = (float)recdat.data[0]*16/10000;
      thermalManager.updatePID();
    break;                       
    case Hot_Bed_D:
      thermalManager.temp_bed.pid.Kd = (float)recdat.data[0]/1.6;
      thermalManager.updatePID();
    break;                      
    case Vmax_X:
...
void RTS_AutoBedLevelPage()
{
  if(waitway == 3)
  {
    ...
    {
      rtscheck.RTS_SndData(ExchangePageBase + 77, ExchangepageAddr);
    }
    waitway = 0;
  }
}
